...
// Round1 is the first DKG round. It produces messages that must be
// shared with all of the other Cosigners before the second round.
func (c Cosigner) Round1() ([][]byte, error) {
	return helpers.PartyRoutine(nil, c.state)
}

// Round2 is the second DKG round. It requires all of the Cosigners messages
// from the first round, and produces messages for the third round.
func (c Cosigner) Round2(round1Msgs [][]byte) ([][]byte, error) {
	if len(round1Msgs) != len(c.allCosigners) {
		return nil, fmt.Errorf(
			"length of messages (%d) must match number of cosigners (%d) for round 2",
			len(round1Msgs),
			len(c.allCosigners),
		)
	}
...
// Round3 is the final DKG round. It requires all of the Cosigners messages
// from the second round. If successful, the shamir secret share is ready for signing.
func (c Cosigner) Round3(round2Msgs [][]byte) error {
	...
	if len(round2Msgs) != expectedLen {
		return fmt.Errorf(
			"length of messages (%d) must match N*(N-1) (%d where N = total number of cosigners %d) for round 3",
			len(round2Msgs),
			expectedLen,
			total,
		)
