...
func initCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cs {
				p, _ := cmd.Flags().GetStringArray("peers")
	...
	cmd.Flags().StringArrayP("peers", "p", []string{}, "cosigner peer addresses in format tcp://{ip}:{port}|{share-id} i.e. tcp://node-1:2222|2,tcp://node-2:2222|3")
