...
func AddPrometheusMetrics(mux *http.ServeMux) {
	logger := cbftlog.NewTMLogger(cbftlog.NewSyncWriter(os.Stdout)).With("module", "metrics")
...
func EnableDebugAndMetrics(ctx context.Context) {
	logger := cbftlog.NewTMLogger(cbftlog.NewSyncWriter(os.Stdout)).With("module", "debugserver")
